...
output: 
  html_document:
    self_contained: true      # CAMBIAR a true
    toc: true
    toc_float: 
      collapsed: false
      smooth_scroll: true
...
frecuencias_n_proyectos <- df_raw %>%
  dplyr::count(n_proyectos) %>%  # ← AQUÍ:  Usa dplyr::count()
...
frecuencias_clasificacion_proyecto <- df_raw %>%
  dplyr::count(CLASIFICACION_DEL_PROYECTO) %>%  # ← AQUÍ
  mutate(Porcentaje = round(n / sum(n) * 100, 2)) %>%
...
frecuencias_ratio <- df_raw %>%
  dplyr:: count(RATIO_ACA_PROYECTO_CULMINADO) %>%  # ← AQUÍ
  mutate(Porcentaje = round(n / sum(n) * 100, 2)) %>%
...
frecuencias_actores <- df_raw %>%
  dplyr::count(Clasificacion_Actores_institucionales) %>%  # ← AQUÍ
  mutate(Porcentaje = round(n / sum(n) * 100, 2)) %>%
